...
## ART-12: expressive area proof

The approved match style is being tested in one opt-in Kettle area: warm cream plaster,
green joinery, wood tables, restrained tonal shading and the same painted-face cast.
Full-body meshes extend the match sources with skinned legs and continuous walking;
conversation close-ups reuse the actual match models. Keep POC exports in
`art/expressive_kettle/`, generated by `tools/build_expressive_kettle.py`.
ART-12R refines this area for a second review using the owner's clarified
Hikaru no Go 3 / GameCube reference. Its focus is readable acting and purposeful
resting hands, balanced full-body proportions, visible surface texture and grounding.
Do not use an era label to excuse awkward geometry or movement.
This does not authorize regenerating the entire campaign world before owner review.
[Inspected playable proof](docs/expressive_kettle/README.md).
