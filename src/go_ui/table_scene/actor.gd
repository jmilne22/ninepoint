...
func setup(who: String) -> void:
    ...
    model = load(KettleNextProfile.person_path(who, "res://art/table_scene/%s.glb" % who)).instantiate()
    ...
    if acting != null:
        camera.size = 1.78
        camera.position.y = 1.84
        camera.look_at(Vector3(0, 1.69, 0))
    if animation:
