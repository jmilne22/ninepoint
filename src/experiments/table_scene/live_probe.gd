## Rendered acceptance against the shipped Wren engine, with ordinary match input.
class_name TableSceneLiveProbe
extends Node

func run(scene: Control) -> void:
    var t := TestKit.new()
    t.section("table_scene live Wren")
    for turn in 2:
        if not await _ready_turn(scene):
            push_error("TableScene live probe timed out waiting for Wren")
            get_tree().quit(1)
            return
        ...
        if not await _ready_turn(scene):
            push_error("Wren did not return a live reply")
            get_tree().quit(1)
            return
        t.eq(scene.game.move_number(), before + 2, "keyboard placement receives a live Wren reply")
    t.ok(scene.opponent is GtpOpponent, "trial uses Wren's GTP profile")
    if scene.opponent is GtpOpponent:
        t.ok(scene.opponent.engine_started, "KataGo started")
        t.ok(not scene.opponent.fallback_used, "Wren did not use fallback")
        t.eq(scene.opponent.legal_reply_count, 2, "both Wren replies were legal")
    await get_tree().create_timer(0.6).timeout
    var output := OS.get_environment("OUT")
    DirAccess.make_dir_recursive_absolute(output)
    await RenderingServer.frame_post_draw
    get_viewport().get_texture().get_image().save_png(output.path_join("live_wren.png"))
