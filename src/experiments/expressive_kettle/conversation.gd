...
func handle_exit(exit: Dictionary, data: NpcData) -> void:
    ...
    match kind:
        "start_match":
            ...
            MatchBridge.start_match(request, room.player_point())
        "start_lesson":
            room.get_window().content_scale_size = Vector2i(384,216)
            MatchBridge.start_lesson(str(exit.lesson), room.player_point(), bool(exit.get("track", false)))
        "start_puzzle":
            room.get_window().content_scale_size = Vector2i(384,216)
            MatchBridge.start_puzzle(str(exit.puzzle), room.player_point())
        _:
            room.set_busy(false)
...
func offer_review(index: int) -> void:
    ...
    offer.transform = Transform2D(0, Vector2(2, 2), 0, Vector2.ZERO)
    # Nested review CanvasLayers have independent transforms; size each one too.
    offer.child_entered_tree.connect(func(child: Node):
        if child is CanvasLayer:
            child.transform = Transform2D(0, Vector2(2, 2), 0, Vector2.ZERO))
    room.add_child(offer)
    await offer.closed
    room.set_busy(false)
    if offer.requested_lesson != "":
        room.get_window().content_scale_size = Vector2i(384,216)
        MatchBridge.start_lesson(offer.requested_lesson, room.player_point())
