...
func setup(who: String) -> void:
    ...
    model = load(KettleNextProfile.person_path(who,"res://art/expressive_world/people/%s.glb" % who)).instantiate()
    ...
    face.set_shader_parameter("faces",load("res://art/expressive_world/people/%s_face.png" % who))
    _prepare(model)
    prepare_acting()
    ...
    if acting != null:
        camera.size = 1.65
        camera.look_at(Vector3(0,1.56,0))
