...
def movie(source,name):
    ...
    temporary=OUT/f'.{name}.encoding.mp4'
    try:
        subprocess.run(['ffmpeg','-v','error','-y','-i',source,'-vf','scale=in_range=pc:out_range=tv,format=yuv420p','-c:v','libx264','-threads','4','-preset','fast','-crf','19','-c:a','aac','-b:a','192k','-movflags','+faststart',str(temporary)],check=True)
