...
static func run(t: TestKit) -> void:
    ...
    var quiet := PracticeGuide.observation(game, GoBoard.BLACK)
    t.ok(quiet["text"].contains("Your group at C5 has the fewest liberties, 4"), "quiet guidance names a real group and its liberty count")
    t.ok(quiet["text"].contains("White's group at E3 has 4"), "and the opponent's weakest group")
    t.ok(not quiet["text"].contains("empty space near"), "no advice about unbounded empty space")
